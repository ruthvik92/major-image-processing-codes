...
def detect_single(image): # iamge is a matrix of type cv_8U containing an image where object are detected
	...
	faces = haar_faces.detectMultiScale(image, 
				...
				flags=cv2.CASCADE_SCALE_IMAGE)  #HAAR_SCALE_FACTORis the parameter specifying how much image size is reduced at each.
...
def resize(image):
	...
	return cv2.resize(image, 
					  ...
					  interpolation=cv2.INTER_LANCZOS4) #cv2.INTER_LANCZOS4 selects smooth interpolation technique for resizing an image.
